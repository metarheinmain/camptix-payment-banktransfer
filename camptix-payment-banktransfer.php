...
class CampTix_Payment_Method_Banktransfer extends CampTix_Payment_Method {
	...
	function camptix_init() {
		...
		add_action( 'camptix_payment_pending', array($this, 'payment_pending_information'), 10, 1 );
	...
	function payment_pending_information($attendee) {
		global $camptix;
		
		if(get_post_meta( $attendee->ID, 'tix_payment_method', true ) == $this->id){
			
			$price = $camptix->append_currency( get_post_meta( $attendee->ID, 'tix_order_total', true ) );
			$bankdetails = nl2br($this->options['bankdetails']);
			$bankdetails .= sprintf( "<br />" . __( "Reference: <strong>%s</strong>", 'camptixpaymentbanktransfer' ), get_post_meta( $attendee->ID, 'tix_banktransfer_token', true ));
			$bankdetails .= sprintf( "<br />" . __( "Amount: %s", 'camptixpaymentbanktransfer' ),  $price );
			
			$camptix->notice( 'raw:' . __( "Please transfer your money to the following bank account:", 'camptixpaymentbanktransfer' ) . "<br />" . $bankdetails );
		}
	}
